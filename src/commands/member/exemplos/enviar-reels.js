...
const REEL_ITEMS = [
  {
    ...
    videoUrl: "https://www.tiktok.com/@bot_marc/video/7425441211613220101",
    source: "TT",
  },
  {
    title: "bot_marc",
  ...
  return {
    ...
    unifiedResponse: {
      data: encodeUnifiedResponseData({
        response_id: `bot_marc-reels-${Date.now()}-${randomBytes(6).toString("hex")}`,
...
function buildBotMetadata(sources = [], extraCapabilities = []) {
  return {
    ...
    botResponseId: `bot_marc-reels-${Date.now()}-${randomBytes(6).toString("hex")}`,
